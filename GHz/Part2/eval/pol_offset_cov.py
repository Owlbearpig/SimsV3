...
if __name__ == '__main__':
    ...
    f_offset, delta_offset, _, _, _, _, _ = meas_with_polOffset(-1.5)
    f_offset2, delta_offset2, _, _, _, _, _ = meas_with_polOffset(-2.5)

    plt.figure()
    plt.plot(f / 10 ** 9, delta / np.pi, '.-', label='Messung')
    plt.plot(f_offset / 10 ** 9, delta_offset / np.pi, '.-', label=r'Messung $-1.5\degree$ Pol. Offset')
    #plt.plot(f_offset2 / 10 ** 9, delta_offset2 / np.pi, '.-', label=r'Messung $-2.5\degree$ Pol. Offset')
